...
function FestivalCard({
  ...
  // (불필요하게 매번 호출되지 않게 useCallback, 조건으로 최적화)
  useCallback(() => {
    if (!infoLoading && !periodLoading && (overview || formattedPeriod)) {
      onUpdateDetails((prev) =>
        prev[festival.id]?.period === formattedPeriod && prev[festival.id]?.description === overview
          ? prev
          : { ...prev, [festival.id]: { period: formattedPeriod, description: overview } }
      );
    }
  }, [infoLoading, periodLoading, formattedPeriod, overview, festival.id, onUpdateDetails])();
